...
function incrementVersionString(version) {
    ...
    const match = raw.match(/^v\s*(.*)$/i);
    const numeric = match ? match[1].trim() : raw.replace(/^v\s*/i, '').trim();
    const segments = numeric ? numeric.split('.') : ['0'];
    const lastIndex = segments.length - 1;
    const parsed = parseInt(segments[lastIndex], 10);

    if (Number.isFinite(parsed)) {
        segments[lastIndex] = String(parsed + 1);
    } else {
        segments.push('1');
    }
...
function getUniqueVersionForParent(parentId, candidate) {
    const used = new Set(getVersionProjectMetaList(parentId).map(meta => String(meta.version || '')));
    let result = String(candidate || 'v 1');
    let guard = 0;

    while (used.has(result) && guard++ < 1000) {
        result = incrementVersionString(result);
    }
...
function sanitizeBasketItemData(data) {
    if (!data) return data;

    if (data.libRef != null) {
        const unchanged = data.isBinary || (typeof data.code === 'string' && data.code === (data.libOriginalCode !== undefined ? data.libOriginalCode : data.code));
...
function sanitizeBasketForStorage(items) {
    return items.map(item => {
        if (!item) return item;

        if (item.type === 'file') {
            return { ...item, data: sanitizeBasketItemData(item.data) };
        }

        if (item.type === 'folder') {
            const filesCopy = {};
            if (item.files) {
                for (const path in item.files) {
                    filesCopy[path] = sanitizeBasketItemData(item.files[path]);
                }
            }
            return { ...item, files: filesCopy };
        }
...
function saveBasket() {
    localStorage.setItem('codium_basket', JSON.stringify(sanitizeBasketForStorage(basket)));
    ...
    switch (item.type) {
        case 'file':
            ...
            if (data.isBinary) {
                ...
                files[item.path] = { ...data, doc: CodeMirror.Doc(data.code || '', mode) };
            }
...
async function createNewVersion() {
    ...
    if (currentProject.parentId) {
        ...
        const nextSiblingVersionString = incrementVersionString(currentVersion);
    ...
    const currentVersion = currentProject.version || 'v 1';
    ...
    let newVersionString = currentProject.version ? incrementVersionString(currentProject.version) : 'v 2';
    newVersionString = getUniqueVersionForParent(currentProject.id, newVersionString);

    // updateCode() applies project meta to the record; make sure the new version is in meta first.
    await updateProjectMetaOnly(currentProjectId, { version: newVersionString });
