...
const mimeTypeBytes = textEncoder.encode(fileData.mimeType || 'application/octet-stream');
parts.push(encodeVarint(mimeTypeBytes.length));
parts.push(mimeTypeBytes);
const contentBytes = fileData.content ? pako.ungzip(fileData.content) : new Uint8Array(0);
...
const resolvedFiles = await resolveProjectFilesForExport(files);
const fileList = Object.keys(resolvedFiles);
const binaryData = serializeProjectOptimized(fileList, resolvedFiles);
